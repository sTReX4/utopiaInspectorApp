...
export default function PersonnelPage() {
  ...
  const [entityToDelete, setEntityToDelete] = useState<{ id: string, name: string, type: 'guard' | 'inspector' } | null>(null);
  ...
  const handleDeleteEntity = async (e: React.FormEvent) => {
    ...
    if (error) {
      alert(`Error deleting ${entityToDelete.type}. They may have historical audits linked to them preventing deletion.`);
      console.error(error);
      return;
    }
      ...
      <div className="enterprise-card overflow-hidden">
        ...
        {activeTab === 'keys' && (
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                <tr><td colSpan={5} className="p-12 text-center text-slate-400 text-sm font-medium">Loading provisioning database...</td></tr>
              ) : filteredKeys.length === 0 ? (
                <tr><td colSpan={5} className="p-12 text-center text-slate-400 text-sm font-medium">No access keys generated yet.</td></tr>
              ) : (
